...
function getshowamount(x){
  var result = 0;
  var basicSalary = parseInt($('#basic').val()); // 45500
  var EachValue = parseInt($('#add_'+x).val()); //30
  var resultant = EachValue * basicSalary/100;
  resultant = resultant || 0;
  document.getElementById('addshow_'+x).value = resultant;
}

function getshowDeducamount(x){
  var result = 0;
  var basicSalary = parseInt($('#basic').val()); // 45500
  var EachValue = parseInt($('#dd_'+x).val()); //30
  var resultants = EachValue * basicSalary/100;
  resultants = resultants || 0;
  document.getElementById('deducshow_'+x).value = resultants;
}


$( document ).ready(function() {
  $( ".addamount" ).each(function( index ) {
  let salaryCounter = $(this).data('id');
  let flag = $(this).data('flag');
  salarySetupsummary(salaryCounter,flag);
  });

  $( ".deducamount" ).each(function( index ) {
    let salaryCounter = $(this).data('id');
    let flag = $(this).data('flag');
    salarySetupsummary(salaryCounter,flag);
...
"use strict";
function salarySetupsummary(x,flag) {
  var addper = 0;
  if(flag == 'add'){
    console.log(flag);
    getshowamount(x);
